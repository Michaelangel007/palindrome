...
History:
 version 13 - remove K&R brace style
 version 12 - added size to DataRate
 version 11 - Converted history to new format
 version 10 - add Print()
 version 9 - add note about missing field initializers: -Wno-missing-field-initializers
 version 8 - add note about uint64_t
 version 7 - split datarate from 1 line into aligned multi-line
 ...
 version 1 - initial version from Buddhabrot
...
*/
    ...
    */
    ...
    // WTF!?!? Why does this exist in winsock2.h ???
...
// Shutup compiler: -Wno-missing-field-initializers
struct DataRate
{
    char     prefix ;
    uint64_t samples;
...
public:
    ...
    // i.e. type is "bytes", "pixels", etc
    void Print( const char *type )
    {
        printf( "Elapsed time: %s%s (%.f seconds)\n", data.day, data.hms, elapsed );
        printf( "Throughput: %llu %c %s/s (%llu samples)\n"
            , throughput.per_sec
            , throughput.prefix
            , type
            , throughput.size
